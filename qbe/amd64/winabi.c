...
static void assign_register_or_stack(RegisterUsage *reg_usage, ArgClass *arg,
                                     bool is_float, bool by_copy) {
...
static void classify_arguments(RegisterUsage *reg_usage, Ins *begin_instr,
                               Ins *end_instr, ArgClass *arg_classes,
                               Ref *env) {
...
static Ins *lower_call(Fn *func, Blk *block, Ins *call_instr,
...
#undef DUP_IF_USED
  ...
  for (Ins *instr = earliest_arg_instr; instr != call_instr; ++instr, ++arg) {
    ...
    case APS_CopyAndPointerOnStack: {
      ...
      arg_copy->instr = (Ins){Oalloc8, Kl, copy_ref, {getcon(arg->size, func)}};
...
static void lower_vastart(Fn *func, RegisterUsage *param_reg_usage,
                          Ref valist) {
  assert(func->vararg);
...
static void lower_args_for_block(Fn *func, Blk *block,
